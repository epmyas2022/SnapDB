...
class PackageStore:
    ...
    def addPackage(self, package_name, bin_path):
        ...
        self.cursor.execute(
            "INSERT INTO Packages (package_name, bin) VALUES (?, ?)",
            (package_name, bin_path),
        )
        self.conn.commit()

    def getActivePackage(self):
        self.cursor.execute("SELECT package_name, bin FROM Packages WHERE active = 1 LIMIT 1")
        result = self.cursor.fetchone()
        if result is None:
            print("\033[31mNo hay paquetes activos. Use 'snapdb use @platform/driver-version' para activar uno.\033[0m")
            raise typer.Exit()
        return {"name": result[0], "bin": result[1]}
    
    def getPackages(self):
        self.cursor.execute("SELECT package_name, active, bin FROM Packages")
        result = self.cursor.fetchall()

        return map(
            lambda x: {"name": x[0], "active": bool(x[1]), "bin": x[2]},
            result,
        )
